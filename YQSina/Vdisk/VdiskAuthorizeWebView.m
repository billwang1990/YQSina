...
- (void)onCloseButtonTouched:(id)sender {
    ...
    if (self.authorize && self.authorize.delegate != nil && [_authorize.delegate respondsToSelector:@selector(authorizeDidCancel:)]) {
        
        [self.authorize.delegate authorizeDidCancel:_authorize];
    }
}
    ...
    if (range.location != NSNotFound) {
        ...
        for (NSString *item in items) {
            ...
            if ([param count] == 2 && [(NSString *)[param objectAtIndex:0] isEqualToString:@"code"]) {
                ...
                if ([self.delegate respondsToSelector:@selector(authorizeWebView:didReceiveAuthorizeCode:)]) {
                    
                    [self.delegate authorizeWebView:self didReceiveAuthorizeCode:code];
                }
